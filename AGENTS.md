...
- `username` 使用当前执行者或系统用户名称；`modelname` 使用真实模型名称，并将不适合作为文件名的空格、斜杠等字符替换为安全分隔符。
- 示例：`20260530-174209-utcp8-taobe-gpt5.md`、`20260530-094209-utcn8-taobe-gpt5.md`。
...
- 当用户通过对话反馈或新增需求，且该需求需要进入核心实现（例如应用代码、业务逻辑、数据结构或界面流程）时，应先将其整理为一条 requirement，再开始执行。新增 requirement 应优先归入现有 Phase；如果适合挂在已有 feature 或已有任务下，应由 Agent 自行判断并归入，不需要额外确认。若需要新建 Phase 或新增三级 feature 标题，由于通常意味着新增 branch/worktree 规划，必须先征询用户确认，再继续执行。
- 条目的命名规则严格按照如下描述：
  ...
  - ### 三级标题总是以 feature 为单位区分，格式形如：`### branch-name：feature description`。
    ...
    - 新增三级 feature 标题通常意味着新增 branch/worktree 规划，必须先征询用户确认；将新 task 归入已有 feature 时，由 Agent 根据语义自行判断即可。
    - 示例：
...
- Agent 不主动创建、删除、移动 worktree，也不主动切换分支；如需调整 worktree 结构、分支关系或工作区位置，必须先征询用户确认。
