...
> Game 子模板的项目专用内容占位。下面所有示例值都需要替换为真实项目信息；不要保留 `例：` 或 `示例：<...>` 字样。

### 项目概况

- 项目名称：例：GameName
- 产品简介：例：一句话说明游戏类型、平台、核心体验。
- 主要用户：例：目标玩家类型、平台。
- 当前阶段：例：原型 / 内部试玩 / 公开测试 / 正式发售。
- 引擎与平台：例：Unity 2023 / Unreal 5 / Godot 4；目标平台 iOS / Android / Steam / Switch 等。

### 技术栈与命令

- 客户端：例：使用的引擎、版本、关键插件、构建工具。
- 服务端：例：使用的语言、框架、数据库、部署平台。
- 资产管线：例：美术资源、音频、关卡数据的组织方式与导入工具。
- 开发命令：例：打开编辑器、启动客户端、启动服务端的命令。
- 测试命令：例：单元测试、Playtest、构建测试包。
- 构建命令：例：构建 Debug / Release 包。
- 发布命令：例：打 tag、提交商店审核、推送 OTA 资源。
...
- 视觉规范：`DESIGN.md`
- 执行日志：`agent-log/`
- 叙事 / 世界观 / 剧本（若使用）：[`universe/AGENTS.md`](file:///Users/taobe/Projects/GitHub/Personal/agent-template/game-agent-template/universe/AGENTS.md)

### 目录索引

- 根目录：`main` 分支工作区。
- `.git/`：主仓库 Git 元数据。
- `worktree/<name>/`：非 `main` 分支的 Git worktree 工作区。
- `client/`：游戏客户端工程（按引擎命名细分，如 `client/unity/`、`client/godot/`）。
- `server/`：游戏服务端工程（若有）。
- `asset/`：美术、音频、动画等非代码资产。
- `universe/`：叙事、世界观、剧本、Wiki 等文字资产。
- `tool/`：内部开发工具、脚本、CLI。
- `agent-log/`：当前工作区执行日志；按需创建 `agent-log/system/` 与 `agent-log/content/`。
- `reference/`（如有）：参考资料，不默认视为项目代码。
- `research/`（如有）：调研文档。
- `.github/workflows/`（如有）：CI、PR check、Release 自动化。

### 子功能文档入口

- [`universe/`](file:///Users/taobe/Projects/GitHub/Personal/agent-template/game-agent-template/universe/AGENTS.md)：游戏世界观、剧本、Wiki 与叙事设计资产；其 `AGENTS.md` 是子规则，遵循上层 `game-agent-template/AGENTS.md` 的「标准内容」。详细规则见 [`universe/AGENTS.md`](file:///Users/taobe/Projects/GitHub/Personal/agent-template/game-agent-template/universe/AGENTS.md)。
- 其他子功能：按需补充。

### Game 类型补充约束

> Game 项目的额外默认约束。如需调整，可在本节说明原因后再修改。

- 默认目录建议：`client/`（游戏客户端工程）、`server/`（游戏服务端工程）、`asset/`（非代码资产）、`universe/`（叙事资产）、`tool/`（内部工具）；不要把游戏工程直接命名为 `unity/`、`unreal/` 这种引擎名作为顶层目录，应放进 `client/` 之下。
- 叙事资产独立：所有剧本、世界观、对话、任务、Wiki 等文字资产必须放在 `universe/`；不得散落到 `client/`、`server/`、`asset/` 下的代码或资源目录中。
- 内容与系统日志拆分：游戏项目同时存在"内容"和"系统"改动时，遵循 `§10` 拆分到 `agent-log/content/` 与 `agent-log/system/`。
...
- 修改 `universe/` 子规则时，先阅读其 [`AGENTS.md`](file:///Users/taobe/Projects/GitHub/Personal/agent-template/game-agent-template/universe/AGENTS.md) 并同步更新本文件中的索引条目。
