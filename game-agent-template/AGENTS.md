...
- 日志中的对话与行动记录应保留完整时间线：用户原始 prompt、补充与纠正、对结果的质疑、双方形成的判断与设计决策、行动结果、测试结果和未解决问题；不能只记录最终代码摘要而省略导致决策的对话。
...
- 每条需求的描述文字必须是自然、直白、清晰、能顺畅读下去的完整句子，读者不需要反复回读或拆解缩写才能看懂；技术标识符（类名、方法名、参数名、数值）可以保留在反引号中，但连接标识符的必须是正常的中文句子，不是电报体或堆砌的短语。
- 标记 `#cut`、`#deferred`、`#blocked` 时，要把完整的前因后果写成一句连贯的话，不要截取一小段原文单独打标签。
- 需求中引入简称或代号时，第一次出现必须顺带说明它具体如何运作，不要只写代号本身；宁可稍微啰嗦，也不要让读者需要额外解码专有名词才能理解需求。
- 不使用复杂表格。
- 不使用 YAML 字段。
...
- 需求文档只收录用户可直接或间接感知的条目：功能、UI、可观察行为、面向用户的文档与质量门槛；纯内部工程事务（清理警告、内部重构、目录整理等）不单独收录为需求，由具体功能任务的完成定义或独立的清理任务处理。
- 工程卫生基础设施（CI、linter、formatter、开发入口、测试基建等）属于用户可间接感知，可登记为顶层条目；但具体测试用例作为所属功能任务的子任务记录，ID 形如 `<parent>-test`（例如 `[0.4.0-DATA-A-001-test]`），完整验收由功能任务、测试子任务与完成定义共同组成。
- 不设 QA、完成定义、验证类顶层条目；「阶段是否完成」性质的区块不进 `REQUIREMENTS.md`，阶段收尾状态由功能任务及其完成定义表达。
...
- 未经当前环境实际运行的验证（编辑器、Playtest、打包、真实浏览器交互等），不得在文档或需求中标记为「已通过」。
- 编译通过、公式复算、物理收敛、回归断言等属于不同验证维度，需分别执行并分别说明，不能以其中一项替代其他；CI 或自动化检查通过不能替代本地实际验证。
- 提交按需求边界划分：一个需求完成并通过对应验证后独立提交，commit message 携带该需求的稳定 ID；不得在一个长期未提交的工作区中并行堆积多个需求，导致改动无法按需求边界回溯或回滚。
- 纯文档重组、纯参数调优与行为变更尽量分开提交。
- 默认只创建本地提交，不自动执行 push、submit 等发布动作，除非用户明确要求。
- 静态分析无法确认的内容（如运行时行为、数据状态）必须标注为推断，不得作为精确事实输出。
- 注释应解释设计意图（WHY）而非单纯复述行为（WHAT）；可调参数的注释必须写清默认值的实际效果和调整范围；修改默认值、公式或安全上限时，必须同步更新注释。
- 重命名、移动或编号类改动后，必须全局检查相关引用、链接、脚本参数和文档示例，不留失效引用；涉及编号的条目检查跳号、重复与顺序一致性。
- 复杂流程按阶段执行，每阶段定义显式的「退出条件」，核对通过后才进入下一阶段。
...
- 文件与目录名应兼容 Git 与跨平台文件系统：不使用空格、中文路径名、大小写混合或特殊符号；目录按需创建，不预建空目录。
...
- 版本号只认一个权威来源：由项目指定单一文件（如根目录 `VERSION` 文件或 `package.json.version`）承载，脚本与 CI 统一从该来源读取，不在多处平行维护。
- 产生新的可交付改动时，按语义化规则递增版本号：破坏性变更递增 major，向后兼容的新能力递增 minor，修复、文案、样式或依赖调整递增 patch。
- 无法明确判断是否递增时，默认递增；漏递增导致版本重复的代价远大于多跳一位。
- 递增后必须同步检查所有对外版本展示（README、界面显示、接口返回等），不允许遗留与权威来源不一致的硬编码版本。
- 每次递增版本号，必须在当次执行日志中写明改了哪一位、依据什么判断。

### 12. UE 引擎项目专门要求

> 本节仅适用于使用 Unreal Engine 的项目；使用其他引擎或非引擎项目的仓库可整体忽略本节。

- 工具链访问纪律：每个任务执行前先检查 Rider / UE 编辑器 MCP 是否可访问；如果任一 MCP 不可用，立即停止任务，并询问用户是否改用其他渠道访问工程。
  - 静态事实优先从仓库文件获取：简单的文本搜索、代码阅读和配置检查使用仓库文件与 `rg`，不为读取静态内容启动 MCP。
  - 写操作前先确认编辑器会话已连接且处于可写状态；写操作后重新读取节点或资源并检查日志，确认写入生效。
  - 覆盖二进制资产前先检查工作区是否已有未提交改动；目标文件属于用户或其他任务的改动时，保留备份、明确覆盖范围，并只验证用户本次指定的资产。
- 调试无响应排查顺序：执行 UE / Rider 相关操作长时间无响应但相关进程未崩溃时，先通过 Rider MCP 检查调试会话是否命中断点，不要把断点暂停误判为引擎卡死或 MCP 失联；若短时间内反复在同一位置命中同一断点，判断运行可能已卡死，此时退出运行或调试会话并告知用户，不反复继续，也不强制结束 UE Editor。
- 引擎 / 编辑器启动默认值：通过 Rider 重启或重新启动 Unreal Editor 时，默认使用 `Development` 构建配置和 `Debug` 模式；只有用户明确要求时才切换为 `DebugGame`、`Shipping`、`Test` 等配置或非 Debug 启动模式。

### 13. Godot 引擎项目专门要求

> 本节仅适用于使用 Godot 的项目；使用其他引擎或非引擎项目可整体忽略本节。

- 测试执行默认使用 Godot Headless：除非该项验证在无头模式下无法实现，或用户明确要求使用编辑器、窗口化运行、Playtest 等方式，否则应使用项目实际配置的 Godot 可执行文件及其 `--headless` 参数运行测试、脚本、场景启动和回归检查。
- 命令必须以项目实际 Godot 版本、可执行文件路径和测试入口为准（例如 `godot --headless --path <project-path> ...`）；示例命令不是固定命令。
- 如果必须改用非 Headless 验证，应在执行日志中说明 Headless 无法覆盖的具体原因、替代验证方式和仍未覆盖的风险；Headless 通过也不能替代需要图形、输入、音频或渲染设备的人工验证。
