...
3. 每次任务开始时，应检查 `source` 指向的模板文件是否有更新。如果 source 的模板版本或通用内容发生更新，应先按以下规则同步通用部分，再重新阅读更新后的 `AGENTS.md` 和相关文档：
1. 同步时，只同步 source 中的通用规则和技术栈规则，保留项目专用规则。
2. 如果本地存在未提交改动，且与 source 发生冲突，不得静默覆盖；应保留本地改动、记录冲突，并请求用户决定合并方式。
3. 如果 source 无法访问，应记录无法检查的原因和剩余风险，不得声称已确认 source 没有更新，并告知用户。
