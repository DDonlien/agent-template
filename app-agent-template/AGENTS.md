...
- 项目名称：例：AppName
- 产品简介：例：一句话说明产品形态和核心价值。
- 主要用户：例：哪类用户会使用这个应用。
- 当前阶段：例：MVP / 内部试用 / 公开 Beta / 正式发布。

### 技术栈与命令

- 技术栈：例：客户端语言与框架、服务端语言与框架、数据库、关键依赖。
- 统一入口：例：优先使用 `Makefile`、根目录脚本或 IDE 工作区。
- 开发命令：例：`make dev`、等价命令。
- 测试命令：例：`make test`、等价命令。
- 构建命令：例：`make build`、等价命令。
- 打包命令：例：`make package`、等价命令。
- 发布命令：例：`make deploy`、等价命令。
...
- `<deliverable>/`：每个交付物目录（按 §8 命名），如 `macos/`、`ios/`、`web/`、`site/`、`admin/`。
- `agent-log/`：当前工作区执行日志。
...
- `<sub-deliverable>/`：每个子交付物或子功能；列出其 `AGENTS.md` / `README.md` / `REQUIREMENTS.md` / `DESIGN.md` / `agent-log/` 入口。

### App 类型补充约束

> App 项目的额外默认约束。如需调整，可在本节说明原因后再修改。

- 默认目录建议：`ios/`（Swift 原生 iOS 应用）、`macos/`（macOS 原生应用）、`web/`（真正的 Web App）、`site/`（官网 / 落地页）、`admin/`（管理后台）；同一交付物下细分前后端时使用 `<deliverable>/frontend/` 与 `<deliverable>/backend/`。
- 本地优先的数据策略：用户凭证、token、订阅状态、本地缓存以本地存储为准；不得把用户私密数据上传到与项目无关的外部服务器。
- 第三方 Provider / SDK：集成前在 `REQUIREMENTS.md` 中增加需求条目；任何对外请求需记录在 `agent-log/` 的验证方式中，便于复盘。
- 凭证与日志安全：浏览器 Cookie、Keychain、CLI 日志和本地配置文件只读访问；日志和控制台输出不得包含 token、cookie、API key 或完整个人账号标识。
- 引用边界：`reference/` 与外部参考项目不默认视为项目代码，引用第三方实现时保留来源说明。
